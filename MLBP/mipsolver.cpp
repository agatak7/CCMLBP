...
//		cplex.setParam(IloCplex::Param::MIP::Limits::Nodes, 1); // Stop after root node.
		...
		SOUT() << "CPLEX status: " << stat << std::endl;
		if (stat == IloAlgorithm::Optimal || stat == IloAlgorithm::Feasible) {
			formulation->extractSolution(cplex, inst, sol);
			SOUT() << "Solution: \n" << sol << std::endl;
			SOUT() << "Objective value: " << cplex.getObjValue() << std::endl;
			SOUT() << "Lower Bound: " << cplex.getBestObjValue() << std::endl;
			SOUT() << "Branch-and-Bound nodes: " << m_bab_nodes << std::endl;
