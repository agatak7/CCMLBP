...
void NF_CCMLBPFormulation::createDecisionVariables(IloEnv env, const Instance<CCMLBP>& inst)
{
	SOUT() << "HELLO " << std::endl;
...
void NF_CCMLBPFormulation::addConstraints(IloEnv env, IloModel model, const Instance<CCMLBP>& inst)
{
...
void NF_CCMLBPFormulation::addObjectiveFunction(IloEnv env, IloModel model, const Instance<CCMLBP>& inst)
{
	IloExpr sum(env);
	for (int k : inst.M) if (k > 0) {
...
void NF_CCMLBPFormulation::extractSolution(IloCplex cplex, const Instance<CCMLBP>& inst, Solution<CCMLBP>& sol)
{
